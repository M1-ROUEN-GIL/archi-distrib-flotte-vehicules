...
\begin{itemize}[leftmargin=*]
  ...
  \item Le chemin HTTP relatif \texttt{/auth} (\texttt{KC\_HTTP\_RELATIVE\_PATH=/auth}) permettant son exposition sous \texttt{flotte.local/auth} via l'ingress Nginx.
\end{itemize}
...
Un script shell \texttt{test-e2e.sh} plac\'{e} \`{a} la racine du projet permet de lancer les tests selon l'environnement cible. Il v\'{e}rifie la disponibilit\'{e} de l'application avant de d\'{e}marrer Playwright et ouvre automatiquement le rapport HTML en fin d'ex\'{e}cution.
