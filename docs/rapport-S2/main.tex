...
\begin{enumerate}[leftmargin=*, label=\textbf{\arabic*.}]
  ...
  \item Mise en place d'une architecture \textbf{Helm unifiee} pour l'integralite du cluster
  \item Centralisation de la stack d'observabilite (\textbf{OpenTelemetry, Jaeger, Prometheus, Loki, Grafana})
  \item Unification des configurations entre Docker Compose et Kubernetes via Helm
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabular}{llll}
    ...
    \midrule
    flotte-postgres-vehicle  & timescale/timescaledb:2.14.2-pg15    & 5432 & Base de donnees principale + time-series \\
...
spring:
  datasource:
    url: ${SPRING_DATASOURCE_URL:jdbc:postgresql://localhost:5432/vehicle_db}
...
\subsection{Strategie de deploiement unifiee}

Le projet a migre vers une architecture **Full Helm**. Plutot que de separer les manifests des microservices de la configuration de l'infrastructure, nous avons cree trois Charts principaux :

\begin{itemize}
  \item \textbf{Infrastructure de base} (\texttt{fleet-infra}) : PostgreSQL, Redis via Bitnami, et Kafka (manifest KRaft).
  \item \textbf{Stack d'observabilite} (\texttt{fleet-observability}) : Toute la stack LGTMe (Loki, Grafana, Tempo, Prometheus) et l'OTel Collector.
  \item \textbf{Application} (\texttt{fleet-app}) : Contient uniquement nos microservices metiers et les regles d'Ingress.
\end{itemize}

Cette separation en trois entites distinctes permet une gestion independante du cycle de vie des bases de donnees, du monitoring et du code metier. Les fichiers de configuration (Prometheus, OTel Collector) sont centralises dans le chart d'observabilite et partages entre Docker Compose (via volumes) et Kubernetes (via ConfigMaps generes par Helm).

Cela permet a n'importe quel developpeur de lancer toute l'infrastructure en \textbf{3 commandes seulement} :

\begin{lstlisting}[language=bash, caption=Lancement complet de l'infrastructure]
# 1. Installer l'infra de base (DBs, Kafka)
helm upgrade --install fleet-infra ./infra/helm/fleet-infra/ -n flotte-namespace
# 2. Installer la stack d'observabilite (LGTMe + OTel)
helm upgrade --install fleet-obs ./infra/helm/fleet-observability/ -n flotte-namespace
# 3. Installer les microservices applicatifs
...
\item \textbf{Utilisation des services natifs GitHub :} des instances \textbf{PostgreSQL 15} et \textbf{Kafka 3.7.0} sont lancees dynamiquement pour chaque job afin de valider le contexte Spring Boot lors des tests d'integration.
        \end{itemize}
...
\subsection{Ameliorations futures de la CI/CD}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{jobs.png}
...
% ------------------------------------------------------------
% SECTION 8 - OBSERVABILITE
% ------------------------------------------------------------
\section{Observabilite -- Stack OpenTelemetry \& Supervision}
...
Les fichiers de configuration (OTel Collector et Prometheus) sont centralises dans le dossier \texttt{infra/helm/fleet-observability/config/}. Ces fichiers servent a la fois pour les montages de volume Docker Compose et pour les ConfigMaps Kubernetes via la fonction \texttt{.Files.Get} de Helm.
...
Une configuration specifique (\texttt{root\_url} pour Grafana et \texttt{base-path} pour Jaeger) a ete appliquee pour permettre le fonctionnement correct derriere ces sous-chemins URLs.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
    PostgreSQL           & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-infra}) \\
    Redis                & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-infra}) \\
    Kafka                & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-infra}) \\
    OTel Collector       & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-observability}) \\
    Jaeger / Prometheus  & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-observability}) \\
    Grafana / Loki       & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-observability}) \\
    vehicle-service      & \textcolor{success}{\textbf{Running}} & Helm (\texttt{fleet-app}) \\
    Autres services      & \textcolor{warning}{\textbf{Pending}} & Helm (\texttt{fleet-app}) \\
    \bottomrule
  \end{tabular}
  \caption{Bilan de l'etat des composants unifies sous Helm}
\end{table}
...
racine-du-projet/
|-- compose.yaml                    # Env. de dev (Postgres, Kafka, Keycloak)
|-- compose.override.yaml           # Stack LGTMe (montage volumes config Helm)
|-- infra/
|   |-- helm/
|   |   |-- fleet-infra/            # Chart INFRA (DBs, Kafka)
|   |   |   |-- values.yaml         # Config bases de donnees et bus
|   |   |   `-- templates/          # Kafka, Postgres (si custom)
|   |   |-- fleet-observability/    # Chart OBSERVABILITE (LGTMe + OTel)
|   |   |   |-- values.yaml         # Config logs, traces et metriques
|   |   |   `-- config/             # SOURCE DE VERITE UNIQUE (OTel, Prom)
|   |   `-- fleet-app/              # Chart APPLICATIF (Microservices)
|   |       |-- values.yaml         # Config des services metiers
|   |       `-- templates/          # Microservices, Ingress
|   `-- kubernetes/
|       |-- namespaces/
|       `-- secrets/
\end{lstlisting}
